...
type GDQNACASSimulator <: AbstractSimulator
...
addObserver(sim::GDQNACASSimulator, f::Function) = add_observer(sim.observer, f)
addObserver(sim::GDQNACASSimulator, tag::AbstractString, f::Function) = add_observer(sim.observer, tag, f)
...
function convert(::Type{SimpleGDQNACASSensorInput}, states::Vector{ASWMState})

    states_ = Array(SimpleGDQNACASSRState, length(states))

    for i = 1:length(states)
        states_[i] = GDQNACASState(states[i].t, states[i].x, states[i].y, states[i].h, states[i].vx, states[i].vy, states[i].vh)
    end

    return GDQNACASSensorInput(states_)
end

convert(::Type{GDQNACASInput}, output::GDQNACASSensorOutput) = GDQNCASInput(output.t, output.r, output.r_d, output.a, output.a_d, output.h, output.h_d)

convert(::Type{SimplePRResolutionAdvisory}, RA::GDQNACASResolutionAdvisory) = SimplePRResolutionAdvisory(RA.h_d)
...
function GDQNACAS_thresholds(h)
    if h < 1000
        dmod = 0
        zthr = 0
    elseif h < 2350
